...
TIMEOUT = 5
# 예전엔 이 값이 CLI 기본값으로 조용히 쓰여서, base_url을 깜빡 빠뜨리고 실행해도
# 티 안 나게 localhost:8081로 진단이 나갔다. 이제 base_url은 build_parser()에서
# 필수 인자로 강제하므로, 이 상수는 더 이상 실제 기본값으로 쓰이지 않는다
# (남겨둔 건 다른 곳에서 참고용으로 값이 필요할 때를 대비한 것뿐).
...
def describe_reflection(state, location):
    descriptions = {
        "원문 반영": (
            f"{location}에서 입력한 식별자와 특수문자(<, >, 큰따옴표, 작은따옴표, &, /, 백틱)가 "
            "변환되지 않은 채 확인됨"
        ),
        "HTML Entity Encoding": (
            f"{location}에서 입력한 특수문자가 &lt;, &gt;, &quot; 등의 HTML Entity로 변환되어 확인됨"
        ),
        "변형 반영": (
            f"{location}에서 입력 식별자는 확인됐지만 특수문자 일부가 변형되어 출력 처리 방식을 추가 확인할 필요가 있음"
        ),
        "미반영": f"{location}에서 입력 식별자가 확인되지 않음",
        "확인 실패": f"{location}의 특수문자 처리 상태를 네트워크 오류로 확인하지 못함",
    }
...
def scan_reflected_xss(browser, session, targets):
    ...
    notes, reflected_states, successful, browser_errors, total_tests = [], [], [], 0, 0
    for target in targets:
        probe_token, probe = make_probe("RXSS_PROBE")
        reflected_state = "확인 실패"
        target_label = f"{target.url}::{target.parameter}"
        try:
            ...
            notes.append(describe_reflection(reflected_state, target_label))
        except requests.RequestException:
            reflected_states.append("확인 실패")
            notes.append(describe_reflection("확인 실패", target_label))
...
def scan_stored_xss(browser, session, forms, pages, allow_posts, cleanup_url):
    ...
    if successful:
        status, risk, confidence = "취약", "높음", "확정"
        evidence = (
            f"허용된 POST 저장 입력 지점 {len(parameters)}개에 대표 XSS 페이로드 {len(test_cases)}개를 각각 전송한 결과, "
            f"총 {total}회 중 "
        ...
        status, risk, confidence = "양호", "낮음", "미확정"
        evidence = (
            f"허용된 POST 저장 입력 지점에 대표 Stored XSS 페이로드 {len(test_cases)}개를 각각 전송했으나 "
            f"총 {total}회 모두 저장 반영 및 "
...
def scan_dom_xss(browser, session, pages):
    ...
    recommendation = "location.hash, window.name, postMessage 등 브라우저 입력값을 innerHTML, document.write, eval 같은 위험 DOM Sink에 직접 전달하지 말 것. 화면 출력은 textContent, setAttribute의 안전한 사용, 검증된 sanitizer로 처리하고 CSP를 보조 통제로 적용할 것"
    ...
    target_pages = list(pages)[:MAX_DOM_PAGES]
    for page_url in target_pages:
        base_url = urldefrag(page_url).url
        for _, template in dom_cases:
            for source in ("location.hash", "window.name", "postMessage"):
                total += 1
    ...
    parameters = sorted({item["where"] for item in successful}) or "location.hash; window.name; postMessage"
    if successful:
        status, risk, confidence = "취약", "높음", "확정"
        evidence = (
            f"선별한 HTML 페이지 {len(target_pages)}개에서 location.hash, window.name, postMessage 경로로 "
        ...
        reason = "클라이언트 JavaScript가 location.hash, window.name, postMessage 등 사용자 제어 DOM Source 값을 실행 가능한 DOM Sink로 전달했고, 브라우저에서 alert 실행이 확인됨. 서버 응답 반영이 아니라 브라우저 내부 DOM 처리 과정에서 발생한 DOM-based XSS로 판단함"
    elif browser_errors:
        status, risk, confidence = "N/A", "낮음", "미확정"
        evidence = (
        ...
        reason = "location.hash, window.name, postMessage 기반 입력을 주입했으나 브라우저에서 alert 실행은 확인되지 않음. 현재 범위에서는 사용자 제어 DOM Source가 위험 DOM Sink로 이어지는 흐름이 재현되지 않았으며, 클라이언트 기능이 제한적인 서비스 구조일 가능성이 있음"
    return finalize_report(
        vulnerability, status, risk, evidence, reason, recommendation, successful,
...
def build_parser():
    ...
    parser.add_argument("base_url", help="진단 대상 base URL (필수). 예: http://localhost:8081")
